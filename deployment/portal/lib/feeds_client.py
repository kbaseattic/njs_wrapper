...
class feeds_client:
    ...
    def notify_users_workspace(self, user, message, job_id, app_name):
        """
        ...
        return requests.post(url, json=note,
                             headers={"Authorization": self.admin_token})


def feeds_service_client() -> feeds_client:
...
if __name__ == '__main__':
    fc = feeds_service_client()
    message = "Test"
    fc.notify_users_workspace(user="bsadkhin", message=message, job_id="jobId",
                                  app_name="App_name")
